...
void main( void ) {
    ...
    for(float i = 0.0; i < num; i++) {
        ...
        float a = i * (time+100000.0) * 0.04;
        r+= cos( ((position.x-resolution.x*res/2.0) * cos(a/res) + (position.y-resolution.y*res/2.0) * sin(a/res) + time)/res/res);
        g+= cos( ((position.x-resolution.x*res/2.0) * cos(a*1.001/res) + (position.y-resolution.y*res/2.0) * sin(a*1.001/res) + time)/res/res);
        b+= cos( ((position.x-resolution.x*res/2.0) * cos(a*1.002/res) + (position.y-resolution.y*res/2.0) * sin(a*1.002/res) + time)/res/res);
